  ...
  try {
    const panelNamesArray =
      config.panelNames.length > 0
        ? config.panelNames[0].split(",").map((name) => name.trim())
        : [];

    // Join the array of panel names with ~
    const panelsString: string =
      panelNamesArray.length > 0 ? `&panels=${panelNamesArray.join("~")}` : "";
    ...
    const panelsWithWinUrlPlaceholder = campaignsData
      .filter((campaign) =>
        campaign.campaigns.some((c) => c.winUrl === "PLACEHOLDER")
      )
      .map((campaign) => campaign.panelName);

    if (panelsWithWinUrlPlaceholder.length > 0) {
      const winUrl: string = `${baseUrl}/v1/engine`;
      const panelNames = panelsWithWinUrlPlaceholder.join("~");
      const queryString = `panels=${encodeURIComponent(
        panelNames
      )}&sessionId=${encodeURIComponent(sessionId)}&gameId=${encodeURIComponent(
        config.gameId
      )}`;
      ...
      const responseData: IResponseData = await winUrlResponse.json();
      const winUrlData: IPrebidResponseData = responseData;

      campaignsData.forEach((campaign) => {
        campaign.campaigns.forEach((c) => {
          if (c.winUrl === "PLACEHOLDER") {
            const winUrl = winUrlData.data.find(
              (w) => w.panelName === campaign.panelName
            );
            if (winUrl) {
              c.imagePath = winUrl.campaigns[0].slides[0].imagePath;
              c.winUrl = "NONE";
              c.viewUrl = "NONE";
              c.slide = winUrl.campaigns[0].slides[0];
            }
          }
        });
      });
